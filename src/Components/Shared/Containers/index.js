...
function DivContainer({ item, testId }) {
  ...
  const handleToggle = async () => {
    ...
    const susbscription = subscriptions.filter((subs) => subs.classId._id === idToUpdate);
    ...
    if (toggle) {
      ...
      const memberIds = susbscription[0].members.map((member) => member._id);
      ...
      const memberIds = memberWithoutCurrent.map((member) => member._id);
      await updateSubscriptions(dispatch, subscriptionID, {
        members: memberIds
      });
